 ...
 * @param {boolean} [options.load_env_local_in_test_env] - load .env.local in test environment
 * @return {string[]}
 */
function listDotenvFiles(dirname, options = {}) {
  const {node_env, load_env_local_in_test_env = false} = options;

  return [
    resolve(dirname, '.env.defaults'),
    resolve(dirname, '.env'),
    (Boolean(load_env_local_in_test_env) || node_env !== 'test') && resolve(dirname, '.env.local'),
 ...
 * @param {boolean} [options.silent=false] - suppress all the console outputs except errors and deprecations
 * @param {boolean} [options.load_env_local_in_test_env=false] - load .env.local in test environment
...
function config(options = {}) {
  ...
  try {
    ...
    const existingFiles = (
      listDotenvFiles(path, {
        node_env,
        load_env_local_in_test_env: Boolean(options.load_env_local_in_test_env)
